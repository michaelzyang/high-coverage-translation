...
if __name__ == "__main__":
    ...
    score(gold, pred, args.verbose)
     ...
     # COMPUTE CANDIDATE SET LENGTHS
    import numpy as np
    gold_list_lens = np.array([len(gold_dict) for gold_dict in gold.values()])
    print(f'Candidate list lengths stats:')
    print(f'min {gold_list_lens.min()}, median {np.median(gold_list_lens)}, mean {gold_list_lens.mean()}, '
          f'max {gold_list_lens.max()}, std {gold_list_lens.std()}')

    cand_list_lens = np.array([len(cand_dict) for cand_dict in pred.values()])
    print(f'Candidate list lengths stats:')
    print(f'min {cand_list_lens.min()}, median {np.median(cand_list_lens)}, mean {cand_list_lens.mean()}, '
          f'max {cand_list_lens.max()}, std {cand_list_lens.std()}')
