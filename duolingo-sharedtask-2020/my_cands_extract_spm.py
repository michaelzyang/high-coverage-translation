...
def main(origfile: str, infile: str, outfile: str, candlimit: int):
    ...
    SPM_SRC = 'data/courses/en-ja/raw/enja_spm_models/spm.en.nopretok.model'
    SPM_TGT = 'data/courses/en-ja/raw/enja_spm_models/spm.ja.nopretok.model'
    sp_src = spm.SentencePieceProcessor()
    sp_src.Load(SPM_SRC)
    sp_tgt = spm.SentencePieceProcessor()
    sp_tgt.Load(SPM_TGT)
    sysfile = "sys.out"  # CONTAINS TOP (1) HYP ONLY (FOR BLEU SCORING)
    reffile = "ref.out"  # CONTAINS REFS
    ...
    with open(infile) as f:
        for line in f:
            ...
            if line.startswith("S-"):
                ...
                outd[num].append(sline[1])  # NOTE: FIRST ELEMENT IN outd LIST IS THE SOURCE SENTENCE (en PROMPT)
                first = True
                cands = 0
    ...
    with open(outfile, "w") as out, open(sysfile, "w") as sf, open(reffile, "w") as rf:    
        ...
        for orig_prompt,item in zip(orig_prompts, sorted(outd.items())):
            ...
            firstline = sp_src.DecodePieces(firstline.split())  # DETOKENIZE WITH SENTENCEPIECE
            textID = orig_prompt[0]
            out.write(f"\n{textID}{FIELDSEP}{firstline}\n")

            for line in linelist[1:]:
                line = sp_tgt.DecodePieces(line.split()) + '\n'  # DETOKENIZE WITH SENTENCEPIECE
                out.write(line)
